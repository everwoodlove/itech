...
class Compilation extends Tapable {
	...
	seal(callback) {
		...
		self.applyPluginsAsyncSeries("optimize-tree", self.chunks, self.modules, function sealPart2(err) {
			...
			if(self.applyPluginsBailResult("should-generate-chunk-photos") !== false) {
			...
			self.applyPluginsAsync("additional-photos", err => {
				if(err) {
					return callback(err);
				}
				self.applyPluginsAsync("optimize-chunk-photos", self.chunks, err => {
					if(err) {
						return callback(err);
					}
					self.applyPlugins1("after-optimize-chunk-photos", self.chunks);
					self.applyPluginsAsync("optimize-photos", self.assets, err => {
						if(err) {
							return callback(err);
						}
						self.applyPlugins1("after-optimize-photos", self.assets);
